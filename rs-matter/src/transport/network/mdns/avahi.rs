...
use super::{CommissionableFilter, DiscoveredDevice, DiscoveredDevices, PushUnique};
...
pub async fn discover_commissionable(
    ...
    let mut pending_services: heapless::Vec<PendingService, MAX_PENDING_SERVICES> =
        heapless::Vec::new();
        ...
        match select3(item_new_fut, all_for_now_fut, timeout_fut).await {
            Either3::First(Some(signal)) => {
                if let Ok(args) = signal.args() {
                    ...
                    let mut pending = PendingService {
                        interface: args.interface,
                        protocol: args.protocol,
                        name: heapless::String::new(),
                        type_: heapless::String::new(),
                        domain: heapless::String::new(),
                    };
                    let _ = write!(&mut pending.name, "{}", args.name);
                    let _ = write!(&mut pending.type_, "{}", args.type_);
                    let _ = write!(&mut pending.domain, "{}", args.domain);

                    let _ = pending_services.push(pending);
                }
            }
            Either3::First(None) => {
                break;
    ...
    for pending in pending_services {
            ...
            .await
        {
            Ok((
                ...
                // Set instance name
                let _ = write!(&mut device.instance_name, "{}", resolved_name);
