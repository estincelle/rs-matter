...
/// A collection of discovered devices with deduplication support.
pub type DiscoveredDevices = heapless::Vec<DiscoveredDevice, MAX_DISCOVERED_DEVICES>;
...
impl PushUnique for DiscoveredDevices {
    fn push_if_unique(&mut self, device: DiscoveredDevice) -> bool {
...
mod tests {
    ...
    fn push_if_unique_adds_new_device() {
        let mut devices: DiscoveredDevices = heapless::Vec::new();
        let mut device = DiscoveredDevice::default();
        let _ = write!(&mut device.instance_name, "device1");
    ...
    fn push_if_unique_rejects_duplicate() {
        ...
        let _ = write!(&mut device1.instance_name, "device1");
        devices.push_if_unique(device1);

        let mut device2 = DiscoveredDevice::default();
        let _ = write!(&mut device2.instance_name, "device1"); // Same name
    ...
    fn push_if_unique_allows_different_names() {
        ...
        let mut device2 = DiscoveredDevice::default();
        let _ = write!(&mut device2.instance_name, "device2");
    ...
    fn push_if_unique_returns_false_when_full() {
        ...
        for i in 0..MAX_DISCOVERED_DEVICES {
            let mut device = DiscoveredDevice::default();
            let _ = write!(&mut device.instance_name, "device{}", i);
            assert!(devices.push_if_unique(device));
        }

        // Try to add one more
        let mut extra = DiscoveredDevice::default();
        let _ = write!(&mut extra.instance_name, "extra");
