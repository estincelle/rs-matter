...
//! Integration test for unsecured exchange initiation.
//!
//! Runs two rs-matter instances in the same process: a device (responder) and a
//! controller (initiator), connected via localhost UDP.
...
fn test_unsecured_exchange_over_udp() {
    ...
    futures_lite::future::block_on(async {
        ...
        let controller_fut = async {
            ...
            let mut test = pin!(run_exchange_flow(&controller_matter, &crypto, peer_addr));
            ...
            match select(&mut transport, &mut test).await {
                ...
                Either::Second(test_result) => {
                    // Give transport a moment to flush the standalone ACK
        ...
        // Race device and controller — controller finishes first on success
